...
export default class PaginationHandler extends Component {
		...
		return (
			<Pagination>
				...
				{pageNumbers.map((number) => {
					if (number >= parseInt(active) - 3 && number <= parseInt(active) + 3) {
						return (
							<Pagination.Item id={number} active={number == active} onClick={this.pagingHandler}>
								{number}
							</Pagination.Item>
						);
					} else {
	...
	buildComponent = (props, state) => {
		...
		return <div className='pull-right'>{this.renderPageNumbers(pageNumbers, totalPages)}</div>;
	};
